      ...
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-md p-6">
        ...
        {currentProfile && (
          <div className="mt-4 p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
            <h3 className="font-semibold text-blue-900 dark:text-blue-200 mb-2 text-sm sm:text-base">Your Profile Summary:</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 text-xs sm:text-sm">
              {currentProfile.fitnessGoal && (
                <div>
                  <span className="text-blue-700 dark:text-blue-300 font-medium">Goal:</span> {currentProfile.fitnessGoal.replace('_', ' ')}
                </div>
              )}
              {currentProfile.exerciseDaysPerWeek && (
                <div>
                  <span className="text-blue-700 dark:text-blue-300 font-medium">Days/Week:</span> {currentProfile.exerciseDaysPerWeek}
                </div>
              )}
              {currentProfile.experienceLevel && (
                <div>
                  <span className="text-blue-700 dark:text-blue-300 font-medium">Level:</span> {currentProfile.experienceLevel}
                </div>
              )}
              {currentProfile.weight && (
                <div>
                  <span className="text-blue-700 dark:text-blue-300 font-medium">Weight:</span> {currentProfile.weight}kg
                </div>
              )}
            </div>
          </div>
